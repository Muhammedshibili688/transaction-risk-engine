...
> **Production-oriented end-to-end machine learning platform for real-time fraud detection featuring streaming inference, explainable AI, monitoring, investigation APIs, and AWS deployment.****
...
### AWS Deployment Benchmark

The platform was deployed on AWS EC2 (c7i.large) using Docker Compose and benchmarked under a sustained real-time transaction workload.
...
| **Metric** | **Result** |
|---------|--------:|
| Transactions Processed (in 90 seconds) | 8,953 |
| Throughput | 94.66 TPS |
| Average Scoring Latency | 53.49 ms |
| Maximum Scoring Latency | 748.70 ms |
| Precision | 0.78 |
| Recall | 0.85 |
| False Positive Rate | 5.29% |


The benchmark was executed against the deployed AWS environment while the complete streaming pipeline (Redis Streams, FastAPI, Prometheus, Grafana, monitoring, and scoring consumers) was active.
...
The complete platform has been deployed on AWS EC2 using Docker Compose, with Redis Streams providing event-driven communication between microservices and Prometheus/Grafana enabling production-style observability.
...
The platform is fully containerized using Docker Compose and has been successfully deployed on AWS EC2.

Docker Compose orchestrates the complete production-style deployment, enabling all services to communicate through an isolated Docker network.
...
The project includes a custom benchmarking framework that validates system health before collecting performance metrics.

The benchmark measures:

- Transaction throughput (TPS)
- Average and peak scoring latency
- CPU utilization
- Memory utilization
- Precision
- Recall
...
This enables repeatable performance evaluation across different deployment environments.

<p align="center">
<img src="images/benchmark-metrics.png" width="100%">
</p>
...
- Sustained streaming throughput of approximately **95 TPS**
- Real-time Redis Stream processing
- Multi-worker asynchronous scoring pipeline
- SHAP-based explainable fraud predictions
- Production monitoring with Prometheus & Grafana
- Automated benchmark framework for latency, throughput, CPU, memory, and model quality evaluation

The benchmark performs automated system health validation before collecting performance metrics to ensure measurements are taken only after the platform reaches a stable operating state.
