...
class User:
    ...
    def generate_raw_tx(self):
        ...
        if self.fraud_state == "NORMAL":
            ...
            lon = self.current_lon + random.uniform(-0.02, 0.02)
            country = self.home_country
            # normal users use standard or digital merchants
            merchant = random.choice(
                MERCHANTS["standard"] + MERCHANTS["digital"]
            )
            ...
            lon = self.current_lon
            country = self.home_country
            # card testing hits same merchant repeatedly
            ...
            lon = random.uniform(*cp["lon_range"])
            country = fraud_country
            # exploiting hits high risk or luxury merchants
            merchant = random.choice(
